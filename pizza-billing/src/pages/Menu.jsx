    ...
    const fetchPizzas = async () => {
      try {
        ...
        Object.keys(categorizedPizzas).forEach(category => {
          const prices = categorizedPizzas[category].items.map(item => parseFloat(item.price));
      ...
      {Object.entries(pizzaCategories).map(([category, { title, priceRange, items }]) => (
        <div key={category} className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {items.map((pizza) => (
              <div key={pizza.id} className="bg-white p-6 rounded-lg shadow-lg">
                <img 
                  src={pizza.image}
                  alt={pizza.name}
                ...
                <p className="text-2xl font-bold text-red-600 mb-3">${pizza.price}</p>
                <button
