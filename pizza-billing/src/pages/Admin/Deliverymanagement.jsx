  ...
  const OrderCard = ({ order }) => {
    ...
    return (
      <div 
        ...
        onClick={() => setSelectedOrder(order)} // Set selected order on click
      >
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-gray-600">Address: {order.street ? `${order.street}, ${order.city}` : 'N/A'}</p>
            <p className="text-gray-600">Total: ${order.total_amount ? order.total_amount.toFixed(2) : '0.00'}</p>
          </div>
  ...
  return (
    <div className="p-6">
      ...
      {selectedOrder && (
        <OrderDetails 
          order={selectedOrder} 
          onClose={() => setSelectedOrder(null)} // Close order details
        />
      )}
